...
import { ICrudExampleModel } from "~/models/crudExampleModel";
...
export let loader: LoaderFunction = async ({ params, request }) => {
	...
	try {
		const result = await API.getTableData({
			session: session,
			url: CONFIG.baseUrlApi + "/crud-example/list",
			pagination: true,
			page: +page || 0,
			size: +size || 10,
			filters: {
	...
	try {
		...
		if (request.method == "DELETE") {
			reponse = await API.delete(
				session,
				CONFIG.baseUrlApi +
					`/crud-example?crudExampleId=${formData.get("crudExampleId")}`
			);
		}
...
export default function Index(): ReactElement {
			...
			data: (data: ICrudExampleModel, index: number): ReactElement => (
				<td key={index + "action"} className="md:px-2 md:py-3">
					...
					<div className="hidden md:block w-64">
						...
						&nbsp;
						<Link to={`/crud-example/edit/${data.crudExampleId}`}>
					...
					<div className="block md:hidden relative">
						...
						>
							...
							>
								<li>
									<Link
										to={`/crud-example/edit/${data.crudExampleId}`}
	...
	const [modalData, setModalData] = useState<ICrudExampleModel>();
	...
	return (
		<div className="">
			<Breadcrumb title="Crud Example" navigation={navigation} />
			...
			>
				<Form method="delete" onSubmit={submitDeleteData}>
					...
					<strong>{modalData?.crudExampleName}</strong>
